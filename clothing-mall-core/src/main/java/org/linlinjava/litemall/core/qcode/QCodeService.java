...
public class QCodeService {
    ...
    // public String createVerifyCode(String orderId) {
    // try {
    // File file = wxMaService.getQrcodeService().createWxaCodeUnlimit("verify," +
    // orderId, "pages/index/index");
    // FileInputStream inputStream = new FileInputStream(file);
    // byte[] imageData = IOUtils.toByteArray(inputStream);
    // ByteArrayInputStream inputStream2 = new ByteArrayInputStream(imageData);
    // LitemallStorage litemallStorage = storageService.store(inputStream2,
    // imageData.length, "image/jpeg", "VERIFY_" + orderId + ".jpg");
    // return litemallStorage.getUrl();
    // } catch (WxErrorException e) {
    // logger.error(e.getMessage(), e);
    // } catch (FileNotFoundException e) {
