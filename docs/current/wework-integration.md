...
> 更新时间：2026-05-11
> 当前状态：待确认。新小程序尚未关联企业微信，本阶段暂不做推送；本文仅保留为后续恢复企业微信能力时的技术参考。

## 当前边界

- 企业微信未关联新小程序，推送能力暂不推进。
- `litemall_wework_*` 相关数据库配置值未知，暂不补默认值。
- 客服跳转企业微信需单独确认微信客服/企业微信后台绑定、客服链接和 `corpId`，不等同于本文的群发推送能力。
- 后续恢复推送前，需重新核验企业微信接口权限、素材规则和小程序关联状态。
