...
async function create(data) {
  const { name, start_date, end_date, sort_order, enabled } = data
  if (!name || !start_date || !end_date) return response.badArgument()
  if (end_date < start_date) return response.fail(402, '结束日期不能早于开始日期')

  const result = await execute(
    'INSERT INTO clothing_holiday (name, start_date, end_date, sort_order, enabled, add_time, update_time, deleted) VALUES (?, ?, ?, ?, ?, NOW(), NOW(), 0)',
    [name, start_date, end_date, sort_order || 0, enabled !== undefined ? (enabled ? 1 : 0) : 1]
  )
  return response.ok({ id: result.insertId })
}

async function update(data) {
  const { id, name, start_date, end_date, sort_order, enabled } = data
  if (!id) return response.badArgument()
  if (start_date && end_date && end_date < start_date) return response.fail(402, '结束日期不能早于开始日期')
...
async function goods(data) {
  ...
  const rows = await query('SELECT goods_id FROM clothing_holiday_goods WHERE holiday_id = ?', [holidayId])
