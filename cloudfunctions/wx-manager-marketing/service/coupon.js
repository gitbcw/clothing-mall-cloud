...
async function create(data) {
  const { name, type, desc, tag, total, discount, min, limit, status,
    ...
    discountType, itemLimit, goodsType, goodsValue, code, popup } = data
  ...
  const result = await execute(
    `INSERT INTO litemall_coupon
      (name, type, \`desc\`, tag, total, discount, \`min\`, \`limit\`, status,
       ...
       discount_type, item_limit, goods_type, goods_value, code, popup,
    ...
    [
      ...
      goodsValue ? JSON.stringify(goodsValue) : '', code || '', popup || 0,
    ]
  )
...
async function update(data) {
  ...
  if (popup !== undefined) { sets.push('popup = ?'); params.push(popup ? 1 : 0) }
