...
async function create(data) {
  const { name, startDate, endDate, sort_order, enabled } = data
  ...
  const result = await execute(
    ...
    [name, startDate, endDate, sort_order || 0, enabled !== undefined ? (enabled ? 1 : 0) : 1]
  )
  return response.ok({ id: result.insertId })
}

async function update(data) {
  const { id, name, startDate, endDate, sort_order, enabled } = data
  if (!id) return response.badArgument()
